...
static void fillBorders16bit(uint8_t *dstp, int width, int height, int stride, int left, int top, int right, int bottom, int mode) {
   ...
   if (mode == ModeFillMargins) {
      for (y = top; y < height - bottom; y++) {
         memset16(dstp16 + stride*y, (dstp16 + stride*y)[left], left);
         memset16(dstp16 + stride*y + width - right, (dstp16 + stride*y + width - right)[-1], right);
      }
...
public:
	FillBorders(PClip _child, int left, int top, int right, int bottom, int mode)
...
{	
	...
	return new FillBorders(args[0].AsClip(), args[1].AsInt(0), args[2].AsInt(0), args[3].AsInt(0), args[4].AsInt(0), args[5].AsInt(0));
}
...
AVSValue __cdecl Create_FillMargins(AVSValue args, void* user_data, IScriptEnvironment* env)
{
	...
	if (vi.BitsPerComponent() == 32) {
		env->ThrowError("FillMargins: Only constant format 8..16 bit integer input supported.");
	}
